...
class SchoolDetailsController extends Controller
{
    ...
    public function getlinks()
    {
        ...
        $data = Schools::select('user_id', 'name', 'email', 'location', 'whatsapp', 'instagram', 'youtube')->where('school_id', $school_id)->first();
        $logo = asset("storage/uploads/" . $data->user_id) . ".png";
        $data["logo"] = $logo;
        return response()->json($data);
    }
...
//    }
    ...
    public function addschoollogo(Request $request)
    {
        ...
        if ($validatedData->passes()) {
            $request->file('profilepic')->storeAs("public/uploads/", Auth::user()->user_id . '.png');
