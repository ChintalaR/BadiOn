            ...
            <div class="col-sm">
                <div class="card dark">
                    <div class="row d-flex justify-content-center">
                        <form id="uploadprofile" class="d-flex justify-content-center">
                            @csrf
                            <label for="profilepic" class="items-center">
                                <img id="profile" class="card-img-top"
                                    src="{{ asset('storage/uploads/' . Auth::user()->user_id . '.png') }}"
                                    onerror="this.onerror=null;this.src='{{ asset('img/school.png') }}'"
                                    style="width: 18rem;" alt="{{ $profile->name }}">
                            </label>
                            <input type="file" name="profilepic" accept="image/png" id="profilepic"
                                style="display: none">
                        </form>
                    </div>
                    <div class="card-body">
...
@section('jquery')
    <script>
        ...
        $("#uploadprofile").validate({
            submitHandler: function(form, event) {
                event.preventDefault();
                let data = new FormData(form);
                $.ajax({
                    type: 'POST',
                    url: "{{ route('addschoollogo') }}",
                    data: data,
                    dataType: 'JSON',
                    contentType: false,
                    processData: false,
